...
export default function ShelterManagementPage() {
  ...
  const filtered = shelters.filter((s) =>
    `${s.shelter_name} ${s.province ?? ""}`.toLowerCase().includes(search.trim().toLowerCase())
  );
    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 min-h-[75vh] font-prompt text-textMain">
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">เครือข่ายแห่งการดูแล เพื่อให้ทุกชีวิตมีบ้าน</p>
        </div>
        <button
          type="button"
          disabled={loading}
          onClick={() => void load()}
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 bg-white hover:bg-[#FAF7F5] transition text-sm font-medium shadow-sm disabled:opacity-50"
        >
          <i className="fa-solid fa-arrows-rotate"></i> โหลดข้อมูลใหม่
        </button>
      </header>

      {/* สถิติ 4 กล่อง */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-5 mb-8 sm:mb-10">
        <div className="bg-white border border-[#f1e4dc] rounded-2xl p-5 text-center shadow-sm">
          <span className="text-2xl text-[#b46c50] mb-1 block">🏠</span>
          <strong className="font-mali text-2xl sm:text-3xl font-bold block text-textMain">
            {loading || error ? "—" : shelters.length}
          </strong>
          <p className="text-xs sm:text-sm text-gray-500">ศูนย์พักพิงในเครือข่าย</p>
        </div>

        <div className="bg-white border border-[#f1e4dc] rounded-2xl p-5 text-center shadow-sm">
          <span className="text-2xl text-[#b46c50] mb-1 block">🐾</span>
          <strong className="font-mali text-2xl sm:text-3xl font-bold block text-textMain">
            {loading || error || animalError
              ? "—"
              : animals.filter((a) => shelters.some((s) => s.shelter_id === a.shelter_id)).length}
          </strong>
          <p className="text-xs sm:text-sm text-gray-500">สัตว์ในสังกัดเครือข่าย</p>
        </div>

        <div className="bg-white border border-[#f1e4dc] rounded-2xl p-5 text-center shadow-sm opacity-70">
          <span className="text-2xl text-[#b46c50] mb-1 block">📍</span>
          <strong className="font-mali text-2xl sm:text-3xl font-bold block text-gray-400">—</strong>
          <p className="text-xs sm:text-sm text-gray-500">รายงานแจ้งพบสัตว์</p>
          <span className="text-[10px] text-gray-400">ยังไม่เชื่อมข้อมูล</span>
        </div>

        <div className="bg-white border border-[#f1e4dc] rounded-2xl p-5 text-center shadow-sm opacity-70">
          <span className="text-2xl text-[#b46c50] mb-1 block">💖</span>
          <strong className="font-mali text-2xl sm:text-3xl font-bold block text-gray-400">—</strong>
          <p className="text-xs sm:text-sm text-gray-500">การจับคู่และ Care Log</p>
          <span className="text-[10px] text-gray-400">ยังไม่เชื่อมข้อมูล</span>
        </div>
      </div>

      {/* แถบหัวข้อและปุ่มเพิ่มศูนย์ */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
        <h2 className="font-mali font-semibold text-xl sm:text-2xl text-textMain">เครือข่ายศูนย์พักพิง</h2>
        <button
          type="button"
        ...
        >
          <i className="fa-solid fa-plus"></i> เพิ่มศูนย์พักพิง
        </button>
      </div>

      {/* ช่องค้นหา */}
      <div className="max-w-md mb-6">
        <label className="block text-xs text-gray-500 mb-1.5 font-medium">ค้นหาศูนย์หรือจังหวัด</label>
      ...
      {loading && <p className="text-center py-16 text-gray-400 font-mali">กำลังโหลดข้อมูล...</p>}
      {error && (
              ...
              <div className="p-5 sm:p-6 border-t border-gray-100">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
                  <div className="md:col-span-2 space-y-3 text-sm text-gray-600">
                    <p>
                      <span className="font-semibold text-textMain inline-block w-24">📍 ที่อยู่:</span>
                      {shelter.address || "ไม่ได้ระบุ"}
                    </p>
                    <p>
                      <span className="font-semibold text-textMain inline-block w-24">📞 เบอร์โทร:</span>
                      {shelter.contact_phone || "ไม่ได้ระบุ"}
                    </p>
                    <p>
                      <span className="font-semibold text-textMain inline-block w-24">✉️ อีเมล:</span>
                  ...
                  <div className="bg-[#FCFAF8] border border-gray-100 rounded-xl p-4 text-center">
                    <strong className="font-mali text-3xl text-primary font-bold block">
                      {animalError ? "—" : animals.filter((a) => a.shelter_id === shelter.shelter_id).length}
                    </strong>
                    <p className="text-xs text-gray-500 mb-2">สัตว์ที่สังกัดศูนย์นี้</p>
                    <button
                      type="button"
                ...
                <div className="flex justify-end gap-2.5 mt-6 pt-4 border-t border-gray-100">
                  ...
                  >
                    <i className="fa-solid fa-trash-can"></i> ลบศูนย์
                  </button>
                </div>
              </div>
            </details>
      ...
      >
        <form onSubmit={save}>
          ...
          <fieldset disabled={saving} className="space-y-4 text-xs sm:text-sm">
            <div>
              <label className="block text-gray-600 mb-1 font-medium">ชื่อศูนย์พักพิง *</label>
              <input
                autoFocus
                required
                maxLength={200}
                value={draft.shelter_name}
                onChange={(e) => setDraft({ ...draft, shelter_name: e.target.value })}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="block text-gray-600 mb-1 font-medium">จังหวัด</label>
                <input
                  maxLength={100}
              ...
              <div>
                <label className="block text-gray-600 mb-1 font-medium">เบอร์โทรศัพท์</label>
                <input
                  type="tel"
                  maxLength={50}
            ...
            <div>
              <label className="block text-gray-600 mb-1 font-medium">ที่อยู่</label>
              <textarea
                rows={3}
                maxLength={2000}
                value={draft.address}
          ...
          {formError && (
            <p className="text-red-500 text-xs mt-3 bg-red-50 p-2.5 rounded-lg">{formError}</p>
          )}
      ...
      >
        ...
        <p className="text-xs text-gray-400 mb-4">ทั้งหมด {selectedAnimals.length} ตัว</p>
          ...
          <ul className="space-y-2.5 max-h-[45vh] overflow-y-auto pr-1 text-sm">
            {selectedAnimals.map((a) => (
              ...
              >
                ...
                <div>
                  <strong className="font-mali text-textMain block">{a.name || "ยังไม่มีชื่อ"}</strong>
                  <p className="text-xs text-gray-500">
                    {a.species || "ไม่ระบุประเภท"} · {a.gender || "ไม่ระบุเพศ"} · {a.status || "ไม่ระบุสถานะ"}
                  </p>
                </div>
              </li>
            ))}
      ...
      >
        ...
        <h2 className="font-mali font-bold text-lg text-textMain mb-1">ลบศูนย์พักพิง</h2>
